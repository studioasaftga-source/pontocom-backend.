...
// ==========================================
// FUNÇÃO MATEMÁTICA: CALCULAR DISTÂNCIA EM METROS
// HAVERSINE
// ==========================================
...
router.get("/", async (req, res) => {
    ...
    try {
        ...
        let query = `
            ...
            WHERE DATE(
                p.data_hora AT TIME ZONE 'America/Cuiaba'
            ) = DATE(
...
async function buscarHojeHandler(req, res) {
    ...
    if (!idFinal) {
        ...
        return res.status(400).json({
            erro: "Informe o ID do funcionário."
        });
    }
    ...
    try {
        ...
        // ==========================================
        // BUSCA SOMENTE AS BATIDAS DE HOJE
        // CONSIDERANDO O HORÁRIO DE CUIABÁ
        // ==========================================
        const query = `
            SELECT *
            ...
            AND DATE(
                data_hora AT TIME ZONE 'America/Cuiaba'
            ) = DATE(
                NOW() AT TIME ZONE 'America/Cuiaba'
            )
...
// POST /api/ponto
// REGISTRO DE PONTO
// GPS + TRAVA 15 MINUTOS + MODO DEMO
// ==========================================
    ...
    try {
        ...
        if (!isModoDemo) {
            ...
            if (!latitude || !longitude) {
                ...
                return res.status(400).json({
                    erro:
                        "Localização GPS obrigatória. Por favor, ative a localização no seu celular!"
                });
            }
            ...
            if (
                configBd.rows.length === 0 ||
                !configBd.rows[0].latitude ||
                !configBd.rows[0].longitude
            ) {

                return res.status(500).json({
                    erro:
                        "As coordenadas da empresa ainda não foram configuradas no Painel do RH."
                });
            }
            ...
            if (
                ...
                distanciaMetros > raioPermitido
            ) {

                return res.status(403).json({
                    erro:
                        `Fora da área! Você está a ${Math.round(distanciaMetros)} metros da empresa. O máximo permitido é ${raioPermitido} metros.`
                });
            }
        }
        ...
        if (
            ...
            if (
                ...
                return res.status(429).json({
                    erro:
                        `Aguarde mais ${faltam} minuto(s) para registrar um novo ponto.`
                });
            }
        }
        ...
        if (!tipoAutomatico) {
            ...
            switch (hoje.rows.length) {
                ...
                case 0:
                    tipoAutomatico = "ENTRADA";
                    break;

                case 1:
                    tipoAutomatico = "SAIDA_ALMOCO";
                    break;

                case 2:
                    tipoAutomatico = "RETORNO_ALMOCO";
                    break;

                case 3:
                    tipoAutomatico = "SAIDA";
                    break;
        ...
        //
        // IMPORTANTE:
        // O NOW() representa o instante atual.
        // As consultas e exibição são convertidas
        // explicitamente para America/Cuiaba.
        // ==========================================
        const queryInsert = `
        ...
        res.status(201).json({
            ...
            // Retorna também a hora já convertida
            // para Cuiabá para o aplicativo poder
            // utilizar diretamente se precisar.
            hora_cuiaba:
                formatarHora(
                    registro.data_hora
                ),
